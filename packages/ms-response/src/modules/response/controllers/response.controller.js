...
export class ResponseController {
  ...
  register(fastify) {
    ...
    fastify.delete("/responses/session/end/:sessionID", this.endSession.bind(this));

    fastify.post("/responses/session/start/:quizzID", this.startSession.bind(this));

    fastify.get("/responses/session/:sessionId", this.getAllSessionResponses.bind(this));
    fastify.get("/responses/participant/:participantId", this.getAllParticipantResponses.bind(this));
    fastify.get("/responses/question/:questionId", this.getAllQuestionResponses.bind(this));
  }
  ...
  */
  async startSession(request, reply) {
    ...
    const responses = await this.responseService.fetchQuizz(quizzID, hostId, request);
    return reply.send(responses);
  }

  async getAllSessionResponses(request, reply) {
    const { sessionId } = request.params;
    const responses = await this.responseService.getAllSessionResponses(sessionId);
    return reply.send(responses);
  }
  ...
  async getAllParticipantResponses(request, reply) {
    ...
    const responses = await this.responseService.getResponsesByParticipant(participantId, sessionId);
    return reply.send(responses);
  }
  ...
  async getAllQuestionResponses(request, reply) {
    ...
    const { sessionId } = request.query;
    const responses = await this.responseService.getResponsesByQuestion(questionId, sessionId);
    return reply.send(responses);
