...
describe("hookInternalToken (Guard/Hook Unit Test)", () => {
  ...
  it("should return 401 if internal-token header is missing", () => {
    const { request, reply, done } = createExecutionContext();
  ...
  it("should return 401 if internal token is invalid or expired", () => {
    const { request, reply, done } = createExecutionContext({
  ...
  it("should populate request.internalTokenPayload and request.user correctly and call done() if token is valid", () => {
    const { request, reply, done } = createExecutionContext({
